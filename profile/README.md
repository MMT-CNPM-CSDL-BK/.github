# Khóa Mạng Máy Tính và Công nghệ Phần Mềm

## 📅 Lộ trình học Mạng Máy Tính

| Buổi | Deadline | Nội dung chính |
|------|----------|----------------|
| 🟢 **TASK 1** |  | Setup dự án **FE & BE** mô hình **client–server** |
| 🟢 **TASK 2** |  | **Giao thức MQTT** trong **IoT** |
| 🟡 **TASK 3** |  | Hiện thực **BTL 1** – **P2P** kết hợp **client–server** |
| 🟡 **TASK 4** |  | Hiện thực **BTL 2** – **thiết kế hệ thống mạng trên Cisco** |
| 🔴 **TASK 5** |  | **Ôn cuối kỳ (CK)** |

## 📅 Lộ trình học Công nghệ Phần Mềm

| Buổi | Deadline | Nội dung chính |
|------|----------|----------------|
| 🟢 **TASK 1** |  | **Git & GitHub**, **CI/CD** |
| 🟢 **TASK 2** |  | Dùng **AI** để thiết kế **FE** cho **BTL** |
| 🟡 **TASK 3** |  | **BTL**: *Use Case Diagrams* & *Use Case Scenarios* |
| 🟡 **TASK 4** |  | **BTL**: *Activity Diagrams*, *Sequence Diagrams*, *State Machine Diagram* |
| 🔴 **TASK 5** |  | **BTL**: *Deployment Diagrams*, *Component Diagram*, *Class Diagram* & *Method Descriptions* |

---
<p align="center">
  <a href="https://www.facebook.com/Shiba.Vo.Tien">
    <img src="https://img.shields.io/badge/Facebook-1877F2?style=for-the-badge&logo=facebook&logoColor=white" alt="Facebook"/>
  </a>
  <a href="https://www.tiktok.com/@votien_shiba">
    <img src="https://img.shields.io/badge/TikTok-000000?style=for-the-badge&logo=tiktok&logoColor=white" alt="TikTok"/>
  </a>
  <a href="https://www.facebook.com/groups/khmt.ktmt.cse.bku?locale=vi_VN">
    <img src="https://img.shields.io/badge/Facebook%20Group-4267B2?style=for-the-badge&logo=facebook&logoColor=white" alt="Facebook Group"/>
  </a>
  <a href="https://www.facebook.com/CODE.MT.BK">
    <img src="https://img.shields.io/badge/Page%20CODE.MT.BK-0057FF?style=for-the-badge&logo=facebook&logoColor=white" alt="Facebook Page"/>
  </a>
  <a href="https://github.com/VoTienBKU">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub"/>
  </a>
</p>
